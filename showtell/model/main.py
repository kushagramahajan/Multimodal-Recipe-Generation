...
def print_sample(hypotheses, references, test_references,imgs, alphas, k, show_att, losses):
    ...
    img = imgs[0][k] 
    imageio.imwrite(str(k)+'_img.jpg', img)
  
    if show_att:
        image = Image.open(str(k)+'_img.jpg')
        ...
        img = imageio.imread(str(k)+'_img.jpg')
        plt.imshow(img)
        plt.axis('off')
        plt.show()
...
def validate():
    ...
    for i in range(20):    
        print_sample(hypotheses, references, test_references, all_imgs, all_alphas,i+1,False, losses)
